...
namespace BlasterBox {
	 ...
	 void Mp3SourceTests::testSetFile()
	 {
			...
			CPPUNIT_ASSERT_THROW(mp3Source.setFilePath(std::string(DATA_PATH) + "DNE.mp3"),
													 Mp3SourceException);
			CPPUNIT_ASSERT(!mp3Source.hasData());
			CPPUNIT_ASSERT(mp3Source.getFilePath() == std::string());

			/* Test setting a file that isn't an mp3 */
			CPPUNIT_ASSERT_THROW(mp3Source.setFilePath(std::string(DATA_PATH) + "notanmp3.txt"),
													 Mp3SourceException);
			CPPUNIT_ASSERT(!mp3Source.hasData());
			CPPUNIT_ASSERT(mp3Source.getFilePath() == std::string());

			/* Test setting an mp3 file that is bad or corrupted */
			CPPUNIT_ASSERT_THROW(mp3Source.setFilePath(std::string(DATA_PATH) + "badmp3.mp3"),
													 Mp3SourceException);
			CPPUNIT_ASSERT(!mp3Source.hasData());
			CPPUNIT_ASSERT(mp3Source.getFilePath() == std::string());
			...
			CPPUNIT_ASSERT(mp3Source.hasData());
			CPPUNIT_ASSERT(mp3Source.getFilePath() == std::string(DATA_PATH) + "mono.mp3");
			CPPUNIT_ASSERT(mp3Source.getNumChannels() == 1);
			CPPUNIT_ASSERT(mp3Source.getRate() == 44100);

			/* Test a stereo mp3 */
			CPPUNIT_ASSERT_NO_THROW(mp3Source.setFilePath(std::string(DATA_PATH) + "stereo.mp3"));
			CPPUNIT_ASSERT(mp3Source.hasData());
			CPPUNIT_ASSERT(mp3Source.getFilePath() == std::string(DATA_PATH) + "stereo.mp3");
